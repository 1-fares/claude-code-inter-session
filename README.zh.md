...
单用户、单机场景下，建议作为**独立 skill** 安装 —— 命令最短。skill 的斜杠触发名取自其安装**目录名**，因此把 skill 目录软链接为 `is`，命令就是 `/is`:
...
<details>
<summary>替代方案:作为插件安装</summary>

插件提供配置界面(端口、空闲关闭)和类似 marketplace 的更新，代价是命令更长、带命名空间(`/inter-session:inter-session …`)。该仓库为私有,因此需要对其有 git 访问权限:
...
插件模式下 monitor **懒启动** —— 仅在你于某个会话中第一次调用 `/inter-session:inter-session` 命令时启动。如需每次会话打开时都连接，运行 `/inter-session:inter-session auto-start on`(然后 `/reload-plugins`)。
...
命令前缀取决于安装方式:推荐的独立 skill 用 `/is`(下方示例),插件则用 `/inter-session:inter-session`。每个子命令都有简写,因此 `/is send …` 与 `/is s …` 等价。
